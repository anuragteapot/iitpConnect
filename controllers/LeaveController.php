...
class LeaveController extends BaseController
{
    ...
    */
    public function giveLeave()
    {
        if (self::$nol == 'RH') {
            ...
            self::quarantineLeave();
        } elseif (self::$nol == 'MAL') {
            self::maternityLeave();
        } elseif (self::$nol == 'MALA') {
            self::maternityLeaveAdoption();
        } elseif (self::$nol == 'SABL') {
            self::sabbaticalLeave();
        } elseif (self::$nol == 'STUDL') {
            self::studyLeave();
        } elseif (self::$nol == 'EOL') {
            self::extraOrdinaryLeave();
        } elseif (self::$nol == 'CL' || self::$nol == 'SCL' || self::$nol == 'LPW' || self::$nol == 'DL') {
            self::casualLeave();
        } elseif (self::$nol == 'V') {
            self::vacationLeave();
        } elseif (self::$nol == 'l0') {
    ...
    */
    public function studyLeave()
    {
        ...
        // Date of joining. TODO
        $doj = '2010-04-04';

        if (self::getNumYear($doj, self::$date2) < 6) {
            $mess = 'Your 5 years is not completed.';
            $result = array('response' => 'error', 'text' => $mess);
            echo json_encode($result);
            exit();
        }
        ...
        if ($days > ($maxDay-$rows['total'])) {
            $mess = 'You have ' . ($maxDay-$rows['total']) . ' study leave left.';
            $result = array('response' => 'error', 'text' => $mess);
            echo json_encode($result);
            exit();
        } else {
    ...
    * @return  NULL
    */
    public function quarantineLeave()
    {
        $app = new Factory;
        $mysql = $app->getDBO();
        $days = $this->numDays(self::$date1, self::$date2);
        ...
        if ($days > ($maxDay-$rows['total'])) {
            $mess = 'You have ' . ($maxDay-$rows['total']) . ' quarantine leave left.';
            $result = array('response' => 'error', 'text' => $mess);
            echo json_encode($result);
            exit();
        } else {
    ...
    */
    ...
    /**
    * Check weekends between two dates.
    *
    * @return  Boolean if Weekend found.
    */
