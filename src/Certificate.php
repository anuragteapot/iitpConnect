...
if(is_uploaded_file($temp['tmp_name']))
{
    ...
    // Verify extension
    if(!in_array(strtolower(pathinfo($temp['name'], PATHINFO_EXTENSION)), array("gif", "jpg", "png", "jpeg", "pdf")))
    {
    ...
    if(isset($_POST['username']) && isset($_POST['profileimage']))
    {
      ...
      if($res)
      {
        $result = array('response' => 'success', 'text' => 'Certificate uploaded.', 'path' => $imagePath);
        echo json_encode($result);
        return;
      }
      else {
  ...
  {
    $result = array('response' => 'error', 'text' => 'Server error.');
    echo json_encode($result);
    return;
  }
